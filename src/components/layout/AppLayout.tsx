...
export function AppLayout() {
  return (
    <div className="min-h-screen bg-metro-cloud text-slate-900">
      <header className="border-b bg-metro-navy text-white">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-4">
          <h1 className="text-xl font-bold tracking-wide">MetroPulse</h1>
          <span className="text-sm text-slate-200">Realtime WMATA Dashboard</span>
        </div>
      </header>
          ...
          <nav className="flex gap-2 md:flex-col">
            {navItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `rounded-lg px-3 py-2 text-sm font-medium ${isActive ? "bg-metro-navy text-white" : "bg-slate-100 text-slate-700"}`
                }
              >
                {item.label}
              </NavLink>
